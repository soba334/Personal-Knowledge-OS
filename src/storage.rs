...
impl Storage {
    ...
    pub async fn ingest_source(
        ...
        for chunk in chunks {
            ...
            sqlx::query(
                "INSERT INTO jobs (id, owner_id, kind, payload) VALUES ($1,$2,'embed_chunk',$3)",
            )
            .bind(Uuid::now_v7())
            .bind(owner_id)
    ...
    pub async fn search_chunks_lexical(
        &self,
        owner_id: Uuid,
        query: &str,
        limit: i64,
    ...
    pub async fn search_memories_lexical(
        &self,
        owner_id: Uuid,
        query: &str,
        limit: i64,
        use_pgroonga: bool,
    ) -> Result<Vec<RankedMemory>, AppError> {
        let sql = if use_pgroonga {
    ...
    pub async fn approve_memory(
        ...
        let current: Option<String> = sqlx::query_scalar(
            "SELECT status FROM memories WHERE owner_id=$1 AND id=$2 FOR UPDATE",
        )
        .bind(owner_id)
        .bind(id)
        ...
        if current.as_deref() != Some("candidate") {
            return Err(AppError::Conflict(
                "memory must be in candidate status".into(),
            ));
        }
        if let Some(previous) = supersedes_id {
            ...
            if result.rows_affected() != 1 {
                return Err(AppError::Conflict(
                    "superseded memory must exist and be active".into(),
                ));
            }
        }
        ...
        sqlx::query("INSERT INTO jobs (id,owner_id,kind,payload) VALUES ($1,$2,'embed_memory',$3)")
            .bind(Uuid::now_v7())
            .bind(owner_id)
            .bind(serde_json::json!({"memory_id":id}))
            .execute(&mut *tx)
            .await?;
        self.audit_tx(
            &mut tx,
            ...
            "memory",
            id,
            serde_json::json!({"supersedes_id": supersedes_id}),
        )
        .await?;
        tx.commit().await?;
        self.get_memory(owner_id, id).await
    ...
    pub async fn reject_memory(&self, owner_id: Uuid, id: Uuid) -> Result<MemoryRecord, AppError> {
        ...
        self.audit(owner_id, "memory.rejected", "memory", id, Value::Null)
            .await?;
        self.get_memory(owner_id, id).await
    }
    ...
    pub async fn complete_job(&self, id: Uuid) -> Result<(), AppError> {
        sqlx::query("UPDATE jobs SET status='done',lease_until=NULL,updated_at=now() WHERE id=$1")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(())
    ...
    pub async fn fail_job(&self, id: Uuid, attempts: i32, error: &str) -> Result<(), AppError> {
        ...
        sqlx::query("UPDATE jobs SET status=CASE WHEN $2 THEN 'failed' ELSE 'pending' END,available_at=CASE WHEN $2 THEN available_at ELSE now()+make_interval(secs => LEAST(300, (2 ^ LEAST($3,8))::int)) END,lease_until=NULL,last_error=$4,updated_at=now() WHERE id=$1")
            .bind(id).bind(terminal).bind(attempts).bind(error).execute(&self.pool).await?;
        Ok(())
    }

    pub async fn load_chunk_content(&self, owner_id: Uuid, id: Uuid) -> Result<String, AppError> {
        sqlx::query_scalar("SELECT content FROM chunks WHERE owner_id=$1 AND id=$2")
            .bind(owner_id)
            .bind(id)
    ...
    pub async fn set_chunk_embedding(
        ...
        sqlx::query("UPDATE chunks SET embedding=$3,embedding_model=$4,embedded_at=now() WHERE owner_id=$1 AND id=$2")
            .bind(owner_id).bind(id).bind(Vector::from(vector)).bind(model).execute(&self.pool).await?;
        Ok(())
    }
    pub async fn load_memory_statement(
        &self,
        owner_id: Uuid,
        id: Uuid,
    ) -> Result<String, AppError> {
        sqlx::query_scalar(
            "SELECT statement FROM memories WHERE owner_id=$1 AND id=$2 AND status='active'",
        )
        .bind(owner_id)
        .bind(id)
        .fetch_optional(&self.pool)
        .await?
        .ok_or_else(|| AppError::NotFound("active memory".into()))
    }
    ...
    pub async fn record_retrieval(
        ...
        sqlx::query("INSERT INTO retrieval_runs (id,owner_id,query,methods,degraded,latency_ms,top_k) VALUES ($1,$2,$3,$4,$5,$6,$7)")
            .bind(Uuid::now_v7()).bind(owner_id).bind(query).bind(methods).bind(degraded).bind(latency_ms).bind(top_k).execute(&self.pool).await?;
        Ok(())
    }
    ...
    async fn audit(
        ...
        sqlx::query("INSERT INTO audit_log (id,owner_id,event_type,subject_type,subject_id,actor,details) VALUES ($1,$2,$3,$4,$5,'api',$6)")
            .bind(Uuid::now_v7()).bind(owner_id).bind(event_type).bind(subject_type).bind(subject_id).bind(details).execute(&self.pool).await?;
        Ok(())
    }
    async fn audit_tx(
        &self,
        ...
        sqlx::query("INSERT INTO audit_log (id,owner_id,event_type,subject_type,subject_id,actor,details) VALUES ($1,$2,$3,$4,$5,'api',$6)")
            .bind(Uuid::now_v7()).bind(owner_id).bind(event_type).bind(subject_type).bind(subject_id).bind(details).execute(&mut **tx).await?;
        Ok(())
    }
}
...
fn source_from_row(row: &sqlx::postgres::PgRow) -> Result<SourceRecord, AppError> {
    Ok(SourceRecord {
        id: row.try_get("id")?,
        kind: row.try_get("kind")?,
...
fn chunk_from_row(row: &sqlx::postgres::PgRow) -> Result<RankedChunk, AppError> {
    Ok(RankedChunk {
...
fn memory_rank_from_row(row: &sqlx::postgres::PgRow) -> Result<RankedMemory, AppError> {
    Ok(RankedMemory {
        memory_id: row.try_get("id")?,
        statement: row.try_get("statement")?,
        score: row.try_get("score")?,
        evidence_source_ids: row.try_get("evidence")?,
        valid_from: row.try_get("valid_from")?,
    })
}
fn memory_from_row(row: &sqlx::postgres::PgRow) -> Result<MemoryRecord, AppError> {
    Ok(MemoryRecord {
        id: row.try_get("id")?,
