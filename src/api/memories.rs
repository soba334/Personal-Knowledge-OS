...
pub async fn create_candidate(
    ...
    if memory_type.is_empty() || memory_type.len() > 64 {
        return Err(AppError::BadRequest("memory_type must be 1..=64 bytes".into()));
    }
    if statement.is_empty() || statement.chars().count() > 4000 {
        return Err(AppError::BadRequest("statement must be 1..=4000 characters".into()));
    }
    ...
    Ok(Json(
        state.storage.create_memory_candidate(
...
pub async fn approve_memory(
    ...
    Ok(Json(state.storage.approve_memory(state.config.owner_id, id, request.supersedes_id).await?))
}
...
pub async fn reject_memory(
    ...
    Ok(Json(state.storage.reject_memory(state.config.owner_id, id).await?))
}
...
pub async fn get_memory(
    ...
    Ok(Json(state.storage.get_memory(state.config.owner_id, id).await?))
}
...
pub async fn list_memories(
    ...
    Ok(Json(state.storage.list_memories(state.config.owner_id, query.status.as_deref(), limit).await?))
}
