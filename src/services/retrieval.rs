...
use crate::{
    AppState,
    error::AppError,
    models::{SearchHit, SearchRequest, SearchResponse},
};
...
pub async fn search(state: &AppState, request: SearchRequest) -> Result<SearchResponse, AppError> {
    ...
    let limit = request
        .limit
        .unwrap_or(state.config.search_result_limit)
        .clamp(1, 50);
    ...
    let lexical_chunks = match state
        .storage
        .search_chunks_lexical(state.config.owner_id, query, candidates, use_pgroonga)
        .await
    {
        Ok(v) => v,
        Err(err) if use_pgroonga => {
            ...
            state
                .storage
                .search_chunks_lexical(state.config.owner_id, query, candidates, false)
                .await?
        }
    ...
    for (rank, hit) in lexical_chunks.into_iter().enumerate() {
        let entry = merged
            .entry(("source_chunk".into(), hit.chunk_id))
            .or_insert_with(|| Acc {
    ...
    if request.include_memories {
        let lexical_memories = match state
            .storage
            .search_memories_lexical(state.config.owner_id, query, candidates, use_pgroonga)
            .await
        {
            Ok(v) => v,
            Err(err) if use_pgroonga => {
                ...
                state
                    .storage
                    .search_memories_lexical(state.config.owner_id, query, candidates, false)
                    .await?
            }
            Err(err) => return Err(err),
        };
        for (rank, hit) in lexical_memories.into_iter().enumerate() {
            let entry = merged
                .entry(("memory".into(), hit.memory_id))
                .or_insert_with(|| Acc {
                    kind: "memory".into(),
                    id: hit.memory_id,
                    source_id: None,
                    title: None,
                    content: hit.statement.clone(),
                    occurred_at: hit.valid_from,
    ...
    match state.embeddings.embed(query).await {
        Ok(Some(vector)) => {
            ...
            for (rank, hit) in state
                .storage
                .search_chunks_dense(state.config.owner_id, vector.clone(), candidates)
                .await?
                .into_iter()
                .enumerate()
            {
            ...
            if request.include_memories {
                for (rank, hit) in state
                    .storage
                    .search_memories_dense(state.config.owner_id, vector, candidates)
                    .await?
                    .into_iter()
                    .enumerate()
                {
        ...
        Ok(None) => {
            degraded = true;
            methods.push("dense_unavailable".into());
        }
        Err(err) => {
            degraded = true;
            methods.push("dense_failed".into());
            tracing::warn!(error=%err,"embedding search unavailable; continuing lexical-only");
        }
    }
    ...
    let mut hits: Vec<SearchHit> = merged
        ...
        .map(|a| SearchHit {
            ...
            content: a.content,
            score: a.score,
            lexical_score: a.lexical,
            dense_score: a.dense,
            occurred_at: a.occurred_at,
            evidence_source_ids: a.evidence,
        })
        .collect();
