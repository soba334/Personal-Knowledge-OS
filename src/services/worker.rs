...
pub async fn run(state: AppState) -> Result<(), AppError> {
    loop {
        let jobs = state
            .storage
            .lease_jobs(state.config.owner_id, state.config.worker_batch_size)
            .await?;
        if jobs.is_empty() {
        ...
        for job in jobs {
            ...
            match result {
                ...
                Err(err) => {
                    ...
                    state
                        .storage
                        .fail_job(job.id, job.attempts, &err.to_string())
                        .await?;
                }
            }
        }
...
async fn process(
    ...
    match kind {
        "embed_chunk" => {
            ...
            let content = state
                .storage
                .load_chunk_content(state.config.owner_id, id)
                .await?;
        ...
        "embed_memory" => {
            ...
            let statement = state
                .storage
                .load_memory_statement(state.config.owner_id, id)
                .await?;
